_cleanupstar = format ["if (isServer) then
{
	...
	{
			cleanup_looop_infiSTAR = true;
			while {cleanup_looop_infiSTAR} do
			{
				{
...
if (cleanup_star == 0) then 
{
	...
	[_cleanupstar] execVM "DeceptionHax\exec.sqf";
}
else
{
	cleanup_star = 0;
	...
	["
		...
		cleanup_looop_infiSTAR = false; 
		cleanup_looop_infiSTAR = nil; 
