...
_checkcrew = true; 	//if crew in vehicle it will not be added to list/teleported
...
} forEach (Entities "LandVehicle"+ Entities "Air" + Entities "Ship" + Entities "Tank");
...
if (selecteditem!= "exitscript") then
{
	_tped = false;
	{
		...
		_pos = getPos player;
		_vname = gettext (configFile >> 'CfgVehicles' >> (typeof _x) >> 'displayName');
		if ((_vname == selecteditem) and !(_tped) and ((player distance _x) > _checkdist) and (fuel _x >= _minfuel) and (damage _x <= _maxdmg) and !(_checkcrew and (count(crew _x)>0))) then
		{
_x setdamage 0;

_x attachto [(vehicle player),[3.0,5.0,1.5]];
...
_tped = true;
	...
	if _tped then {cutText [format["Here is your %1",selecteditem],"PLAIN DOWN"];titleFadeOut 2;}
	else {cutText [format["All %1 have crew now or unavaliable... Sorry..",selecteditem],"PLAIN DOWN"];titleFadeOut 2;};
};